      ...
      layer.on({
        ...
        click: function (e) {
          ...
          if (makeCircle) {
            ...
            map.setView([props.latitude, props.longitude], 19); // @NOTE: map.flyTo is undefined, for some reason
...
async function fetch_report_data(recordNum) {
  ...
  const stream = await fetch(
    `https://cloud.dvrpc.org/api/traffic-counts/v1/record/${recordNum}`
  );
...
function make_report_table_content(counts) {
  ...
  counts.forEach((count) => {
    ...
    midnightTd.textContent = count.AM12 == null ? '' : count.AM12;
    oneamTd.textContent = count.AM1 == null ? '' : count.AM1;
    twoamTd.textContent = count.AM2 == null ? '' : count.AM2;
    threeamTd.textContent = count.AM3 == null ? '' : count.AM3;
    fouramTd.textContent = count.AM4 == null ? '' : count.AM4;
    fiveamTd.textContent = count.AM5 == null ? '' : count.AM5;
    sixamTd.textContent = count.AM6 == null ? '' : count.AM6;
    sevenamTd.textContent = count.AM7 == null ? '' : count.AM7;
    eightamTd.textContent = count.AM8 == null ? '' : count.AM8;
    nineamTd.textContent = count.AM9 == null ? '' : count.AM9;
    tenamTd.textContent = count.AM10 == null ? '' : count.AM10;
    elevenamTd.textContent = count.AM11 == null ? '' : count.AM11;
    noonTd.textContent = count.PM12 == null ? '' : count.PM12;
    onepmTd.textContent = count.PM1 == null ? '' : count.PM1;
    twopmTd.textContent = count.PM2 == null ? '' : count.PM2;
    threepmTd.textContent = count.PM3 == null ? '' : count.PM3;
    fourpmTd.textContent = count.PM4 == null ? '' : count.PM4;
    fivepmTd.textContent = count.PM5 == null ? '' : count.PM5;
    sixpmTd.textContent = count.PM6 == null ? '' : count.PM6;
    sevenpmTd.textContent = count.PM7 == null ? '' : count.PM7;
    eightpmTd.textContent = count.PM8 == null ? '' : count.PM8;
    ninepmTd.textContent = count.PM9 == null ? '' : count.PM9;
    tenpmTd.textContent = count.PM10 == null ? '' : count.PM10;
    elevenpmTd.textContent = count.PM11 == null ? '' : count.PM11;
    totalTd.textContent = count.total ? count.total.toLocaleString() : 0;
...
function create_sidebar_report(data) {
  ...
  const srOrI = data.SR
    ? "State Route " + data.SR
    : "State Route ID " + data.SRI;

  back.innerHTML = '<span class="btn-back-arrow">&#8592;</span> back to results';
  name.textContent = data.road;
  countContext.innerHTML = `<span class="count-type ${data.count_type.toLowerCase()}">${
    data.count_type
  } count #${
    data.record_num
  }</span>  | ${srOrI} | ${data.municipality.toLowerCase()} | ${data.county.toLowerCase()}`;
  downloadBtn.textContent = "Download as CSV";

  // generate data
  const tableData = make_report_table_content(data.counts);
  const footerData = make_report_footer(data);
