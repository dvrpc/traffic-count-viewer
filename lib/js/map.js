...
function make_report_table_content(counts) {
    ...
    counts.forEach(count => {
        ...
        totalTd.textContent = count.total.toLocaleString() || 'n/a'
...
function make_report_subheader(data) {
    ...
    muni.classList.add('subheader-span')
    ...
    sr.classList.add('subheader-span')
    seq.classList.add('subheader-span')

    id.innerHTML = `<span>Station ID</span> ${data.station_id}`
    ampeak.innerHTML = `<span>AM Peak</span> ${data.am_peak}`
    pmpeak.innerHTML = `<span>PM Peak</span> ${data.pm_peak}`
    muni.innerHTML = `<span>Municipality</span> ${data.municipality}`
    ...
    sr.innerHTML = `<span>SR</span> ${data.SR}`
    seq.innerHTML = `<span>SEQ</span> ${data.SEQ}`

    frag.appendChild(id)
    frag.appendChild(ampeak)
    frag.appendChild(pmpeak)
    frag.appendChild(muni)
