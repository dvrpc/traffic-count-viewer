...
var query_sources = {
  ...
  neighborhood: {
    key: "FID",
    url: "https://services.arcgis.com/fLeGjb7u4uXqeF9q/ArcGIS/rest/services/Philadelphia_Neighborhoods_Reference/FeatureServer/0",
  },
};
...
function processData() {
  ...
  $("#userQuery")
    .typeahead(ttoptions, ttoptions_display)
...
function createMap() {
  ...
  L.Control.MapLegend = L.Control.extend({
    ...
    onAdd: function (map) {
      ...
      var legendDiv = L.DomUtil.create(
        "div",
        "map-legend legend-control leaflet-bar",
      );

      legendDiv.innerHTML +=
        '<div id="legend-icon" title="Toggle Legend"><i class="glyphicon glyphicon-minus"></i><span class="legend-label" style="display:none;">&nbsp;&nbsp;Legend</span></div>';

      var legend_top = L.DomUtil.create(
          "div",
          "map-legend-items legend-top",
          legendDiv,
        ),
        legend_body = L.DomUtil.create(
          "div",
          "map-legend-items legend-body",
          legendDiv,
        ),
        legend_bottom = L.DomUtil.create(
          "div",
          "map-legend-items legend-bottom",
          legendDiv,
        );

      legend_body.innerHTML +=
        '<div id="legend-content" class="row"><div class="col-xs-4"><i class="glyphicon glyphicon-circle ct-vol"></i>&nbsp;&nbsp;Volume<br/><i class="glyphicon glyphicon-circle ct-15min"></i>&nbsp;&nbsp;15 Min Volume<br/><i class="glyphicon glyphicon-circle ct-speed"></i>&nbsp;&nbsp;Speed</div><div class="col-xs-4"><i class="glyphicon glyphicon-circle ct-class"></i>&nbsp;&nbsp;Class<br/><i class="glyphicon glyphicon-circle ct-mclass"></i>&nbsp;&nbsp;Manual Class<br/><i class="glyphicon glyphicon-circle ct-tm"></i>&nbsp;&nbsp;Turning Movement</div><div class="col-xs-4"><i class="glyphicon glyphicon-circle ct-loop"></i>&nbsp;&nbsp;Loop<br/><i class="glyphicon glyphicon-circle ct-8day"></i>&nbsp;&nbsp;8 Day<br/><i class="glyphicon glyphicon-circle ct-bike"></i>&nbsp;&nbsp;Bicycle<br/><i class="glyphicon glyphicon-circle ct-ped"></i>&nbsp;&nbsp;Pedestrian<br/><i class="glyphicon glyphicon-circle ct-cwalk"></i>&nbsp;&nbsp;Crosswalk</div></div>';

      legend_top.innerHTML +=
        '<p><b>Count Types</b><span id="legend-definition" class="nav-item" data-modal="legend-modal"><i class="glyphicon glyphicon-info-sign"></i>&nbsp;&nbsp;data definitions</span>';
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      ...
      layer.on({
        ...
        click: function (e) {
          // clear any existing reports before proceeding
          remove_sidebar_report();

          const props = e.target.feature.properties;
          activeCircleId = e.target._leaflet_id;
          const makeCircle = fetch_report_data(props.recordnum);

          if (makeCircle) {
            syncMapTable = false; // don't recalculate data table

            map.setView([e.latlng.lat, e.latlng.lng], 19); // @NOTE: map.flyTo is undefined, for some reason
...
function fadeLanding(type, layer, value) {
  ...
  $("#landing_map")
    .toggleClass("col-sm-12 col-lg-12 col-sm-6 col-lg-6")
    .delay(790)
    .queue(function () {
      createMap();
...
function map_selected() {
  if (download_records.length) {
    ...
    dwnld_json =
      ...
      JSON.stringify(download_records) +
      "}";
  ...
  results.eachLayer(function (m) {
    var date = m.feature.properties.setdate,
      ...
      report =
        ...
        m.feature.properties.recordnum +
        '&nbsp;<i class="glyphicon glyphicon-open" title="view report"></i></button>',
      unique = L.stamp(m),
      dt = new Date(date),
      latlng = m._latlng,
      tableRoad =
        processRoadname(m.feature.properties) +
        " " +
        (cntdir.length && cntdir != "both"
          ? '<span class="cnt-direction" >- ' +
            cntdir[0].toUpperCase() +
            "B</span>"
          : ""),
      MmYear =
        '<span class="hidden">' +
    ...
    TCtable.push({
      ...
      aadt: type === "Turning Movement" ? "" : aadt_form,
      cntdir: cntdir,
      unid: unique,
      recnum: recnum,
      report: report,
...
function render_table(values) {
  ...
  if (values.length) {
    if (proximity_sort) {
      //if proximity sort is active build sorted by distance
      sortDataTable();
    } else {
      clearTable();
      table.order([1, "desc"]);
    ...
    if (currTable.length != TCtable.length) {
      $("#table_info").append(
        " (" + TCtable.length + " results in search area)",
      );
    }
    build_legend(values);
  } else {
...
function build_legend(v) {
  ...
  legend_sorted = Object.keys(legend_types).sort(function (a, b) {
    return legend_types[a] - legend_types[b];
  });
  ...
  for (var i = 0, len = legend_sorted.length; i < len; i++) {
    ...
    legend_content +=
      '<i class="glyphicon glyphicon-circle ' +
      count_dictionary[legend_sorted[i]].class +
      '"></i>&nbsp;&nbsp;' +
      legend_sorted[i] +
      "</br>";
    if (i + 1 === len) {
...
function cartDist(oLatLng, dLatLng) {
  return Math.sqrt(
    Math.pow(dLatLng.lat - oLatLng.lat, 2) +
      Math.pow(dLatLng.lng - oLatLng.lng, 2),
  );
}
...
async function fetch_report_data(recordNum) {
  ...
  const stream = await fetch(
    `https://cloud.dvrpc.org/api/traffic-counts/v2/volume/hourly/${recordNum}`,
  );
...
function fetch_csv(id) {
  window.open(
    `https://cloud.dvrpc.org/api/traffic-counts/v2/volume/hourly/csv/${id}`,
  );
}
...
function make_report_table_content(counts) {
  ...
  Object.keys(counts).forEach((key) => {
    ...
    if (!ret[date.toISOString().slice(0, 10)]) {
      ...
      ret[date.toISOString().slice(0, 10)].hours[
        date.toLocaleTimeString("en-GB")
      ] = counts[key];
      ret[date.toISOString().slice(0, 10)].total += counts[key];
    }
  });

  // add fetched data
  Object.entries(ret).forEach((count) => {
    // create header cells
    const day = count[0].replace(/-/g, "/").replace(/T.+/, ""); // hack to get the right day of the week
    ...
    midnightTd.textContent =
      count[1].hours["00:00:00"] == null ? "" : count[1].hours["00:00:00"];
    oneamTd.textContent =
      count[1].hours["01:00:00"] == null ? "" : count[1].hours["01:00:00"];
    twoamTd.textContent =
      count[1].hours["02:00:00"] == null ? "" : count[1].hours["02:00:00"];
    threeamTd.textContent =
      count[1].hours["03:00:00"] == null ? "" : count[1].hours["03:00:00"];
    fouramTd.textContent =
      count[1].hours["04:00:00"] == null ? "" : count[1].hours["04:00:00"];
    fiveamTd.textContent =
      count[1].hours["05:00:00"] == null ? "" : count[1].hours["05:00:00"];
    sixamTd.textContent =
      count[1].hours["06:00:00"] == null ? "" : count[1].hours["06:00:00"];
    sevenamTd.textContent =
      count[1].hours["07:00:00"] == null ? "" : count[1].hours["07:00:00"];
    eightamTd.textContent =
      count[1].hours["08:00:00"] == null ? "" : count[1].hours["08:00:00"];
    nineamTd.textContent =
      count[1].hours["09:00:00"] == null ? "" : count[1].hours["09:00:00"];
    tenamTd.textContent =
      count[1].hours["10:00:00"] == null ? "" : count[1].hours["10:00:00"];
    elevenamTd.textContent =
      count[1].hours["11:00:00"] == null ? "" : count[1].hours["11:00:00"];
    noonTd.textContent =
      count[1].hours["12:00:00"] == null ? "" : count[1].hours["12:00:00"];
    onepmTd.textContent =
      count[1].hours["13:00:00"] == null ? "" : count[1].hours["13:00:00"];
    twopmTd.textContent =
      count[1].hours["14:00:00"] == null ? "" : count[1].hours["14:00:00"];
    threepmTd.textContent =
      count[1].hours["15:00:00"] == null ? "" : count[1].hours["15:00:00"];
    fourpmTd.textContent =
      count[1].hours["16:00:00"] == null ? "" : count[1].hours["16:00:00"];
    fivepmTd.textContent =
      count[1].hours["17:00:00"] == null ? "" : count[1].hours["17:00:00"];
    sixpmTd.textContent =
      count[1].hours["18:00:00"] == null ? "" : count[1].hours["18:00:00"];
    sevenpmTd.textContent =
      count[1].hours["19:00:00"] == null ? "" : count[1].hours["19:00:00"];
    eightpmTd.textContent =
      count[1].hours["20:00:00"] == null ? "" : count[1].hours["20:00:00"];
    ninepmTd.textContent =
      count[1].hours["21:00:00"] == null ? "" : count[1].hours["21:00:00"];
    tenpmTd.textContent =
      count[1].hours["22:00:00"] == null ? "" : count[1].hours["22:00:00"];
    elevenpmTd.textContent =
      count[1].hours["23:00:00"] == null ? "" : count[1].hours["23:00:00"];
    totalTd.textContent = count[1].total;

    highTd.textContent = count.high_temp || "N/A";
    lowTd.textContent = count.low_temp || "N/A";
    weatherTd.textContent = count.weather || "N/A";
...
function make_report_footer(data) {
  const frag = document.createDocumentFragment();
  ...
  id.innerHTML = `<span>Station ID:</span> ${data.station_id || "N/A"}`;
  ampeak.innerHTML = `<span>AM Peak:</span> ${data.am_peak || "N/A"}, ${
    data.am_ending || "N/A"
  }`;
  pmpeak.innerHTML = `<span>PM Peak:</span> ${data.pm_peak || "N/A"}, ${
    data.pm_ending || "N/A"
  }`;
  project.innerHTML = `<span>Project:</span> ${data.project || "N/A"}`;
  countDir.innerHTML = `<span>Count Direction:</span> ${
    data.counter_direction || "N/A"
  }`;
  trafficDir.innerHTML = `<span>Traffic Direction:</span> ${
    data.traffic_direction || "N/A"
  }`;
  axle.innerHTML = `<span>Axle:</span> ${data.AXLE || "N/A"}`;
  seq.innerHTML = `<span>Segment:</span> ${data.SEQ || "N/A"}`;
  offset.innerHTML = `<span>Offset:</span> ${data.offset || "N/A"}`;

  // handle whatever this is
  if (data.AADT) {
    aadJawn.innerHTML = `<span>AADT</span> ${data.AADT || "N/A"}`;
  } else if (data.AADB) {
    aadJawn.innerHTML = `<span>AADB</span> ${data.AADB || "N/A"}`;
  } else if (data.AADP) {
    aadJawn.innerHTML = `<span>AADP</span> ${data.AADP || "N/A"}`;
  }
...
function create_sidebar_report(data) {
  ...
  back.classList.add("btn", "btn-secondary", "btn-back");
  ...
  // text
  const srOrI = metadata.sr
    ? "State Route " + metadata.sr
    : "State Route ID " + metadata.sri;

  back.innerHTML =
    '<span class="btn-back-arrow">&#8592;</span> back to results';
  name.textContent = metadata.road;
  countContext.innerHTML = `<span class="count-type ${metadata.count_type.toLowerCase()}">${
    metadata.count_type
  } count #${
    metadata.record_num
  }</span>  | ${srOrI} | ${metadata.municipality.toLowerCase()} | ${metadata.county.toLowerCase()}`;
  downloadBtn.textContent = "Download as CSV";

  // generate data
  const tableData = make_report_table_content(data.counts);
  const footerData = make_report_footer(metadata);

  // event handlers
  back.onclick = () => back_to_results();
  downloadBtn.onclick = () => fetch_csv(metadata.record_num);
  ...
  report.appendChild(tableFooter);
...
$(document.body).on("click", "#table tbody td", function () {
  if (table.cell(this).index().column === 4) {
    var trow = table.cell(this).index().row;
    var datarow = table.row(trow).data();
...
$(document.body).on("click", "#legend-icon", function () {
  ...
  if (toggleStatus === "closed") {
    ...
    $("#legend-icon i").toggleClass(
      "glyphicon glyphicon-list glyphicon glyphicon-minus",
    );
    ...
    $("#legend-icon i").toggleClass(
      "glyphicon glyphicon-minus glyphicon glyphicon-list",
    );
...
function partyTime(dude) {
  ...
  tasty.id = "party";
  tasty.style =
    "width:500px;height:60px;position:absolute;top:130px;left:20px;background-color:#efefef;";
  var shut_it = document.createElement("div");
  shut_it.id = "js-sc-close";
  shut_it.style =
    "height:20px;width:20px;font-size:14px;color:#fff;text-align:center;background-color:#ff9900;position:absolute;top:0;right:-20px;";
  shut_it.innerHTML = "x";
  ...
  if (dude === "tasty time") {
    iframe.src =
      "https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/269256410&amp;color=ff9900&amp;auto_play=true&amp;hide_related=false&amp;show_comments=false&amp;show_user=false&amp;show_reposts=false;";
  } else if (dude === "gusgus") {
    iframe.src =
      "https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/18608884&amp;color=ff5500&amp;auto_play=true&amp;hide_related=false&amp;show_comments=false&amp;show_user=false&amp;show_reposts=false";
  } else if (dude === "goodbye brady") {
    iframe.src =
      "https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/108456959&amp;color=ff5500&amp;auto_play=true&amp;hide_related=false&amp;show_comments=false&amp;show_user=false&amp;show_reposts=false";
  }
...
$(window).resize(function () {
  if (tableExists && TCtable.length) {
    table.page
      .len(Math.max(1, Math.floor(getDynTableHeight() - 32) / 38.5 - 1))
      .draw(); // subtract padding and do other things
  }
});
