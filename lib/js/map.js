...
function make_report_table_content(counts) {
  ...
  Object.keys(counts).forEach((key) => {
    ...
    ret[date].hours[time] = counts[key];
    ret[date].total += counts[key];
  });
...
function make_report_footer(data) {
  ...
  ampeak.innerHTML = `<span>AM Peak:</span> ${data.am_peak_volume || "N/A"}`;
  pmpeak.innerHTML = `<span>PM Peak:</span> ${data.pm_peak_volume || "N/A"}`;
