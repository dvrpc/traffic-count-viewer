...
// locationQuery = [],
    ...
    // @UPDATE: btns have padding top and bottom of 3 now so 6
...
// @UPDATE: remove "view selected" btn option
...
function map_results() {
    ...
    // @UPDATE: remove "view selected" option
...
function make_report_table_content(counts) {
    ...
    counts.forEach(count => {
        // create header cells
        // @UPDATE: pretty sure these days are all 1 day behind b/c JS Date()...
        const day = count.date
        ...
        midnightTd.textContent = count.AM12 || 'n/a'
        oneamTd.textContent = count.AM1 || 'n/a'
        twoamTd.textContent = count.AM2 || 'n/a'
        threeamTd.textContent = count.AM3 || 'n/a'
        fouramTd.textContent = count.AM4 || 'n/a'
        fiveamTd.textContent = count.AM5 || 'n/a'
        sixamTd.textContent = count.AM6 || 'n/a'
        sevenamTd.textContent = count.AM7 || 'n/a'
        eightamTd.textContent = count.AM8 || 'n/a'
        nineamTd.textContent = count.AM9 || 'n/a'
        tenamTd.textContent = count.AM10 || 'n/a'
        elevenamTd.textContent = count.AM11 || 'n/a'
        noonTd.textContent = count.PM12 || 'n/a'
        onepmTd.textContent = count.PM1 || 'n/a'
        twopmTd.textContent = count.PM2 || 'n/a'
        threepmTd.textContent = count.PM3 || 'n/a'
        fourpmTd.textContent = count.PM4 || 'n/a'
        fivepmTd.textContent = count.PM5 || 'n/a'
        sixpmTd.textContent = count.PM6 || 'n/a'
        sevenpmTd.textContent = count.PM7 || 'n/a'
        eightpmTd.textContent = count.PM8 || 'n/a'
        ninepmTd.textContent = count.PM9 || 'n/a'
        tenpmTd.textContent = count.PM10 || 'n/a'
        elevenpmTd.textContent = count.PM11 || 'n/a'
...
// generate misc. report metadata (pending fields)
function make_report_subheader(data) {
    ...
    id.innerHTML = `<strong>Station ID</strong> ${data.station_id}`
    project.innerHTML = `<strong>Project</strong> ${data.project}`
    countDir.innerHTML = `<strong>Count Direction</strong> ${data.counter_direction}`
    trafficDir.innerHTML = `<strong>Count Direction</strong> ${data.traffic_direction}`
...
function create_sidebar_report(data) {
    ...
    countType.textContent = data.count_type + ' count'
...
function add_sidebar_report() {
    // hide data table
        // @TODO: highlight selected count on the map
    ...
    const dataTest = {
        ...
        "SRI": null,
        "MP": null,
        "latitude": 40.13079826,
        "longitude": -75.37048106,
        "MCD": "4209183696",
        "municipality": "WEST NORRITON TWP",
        "county": "MONTGOMERY",
        "state": "PA",
        "route": null,
        "road": "whitehall rd",
    ...
    // END sample resposne for testing
