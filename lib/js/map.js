...
function make_report_footer(data) {
    ...
    ampeak.innerHTML = `<span>AM Peak:</span> ${data.am_peak || 'N/A'}, ${data.am_ending || 'N/A'}`
    pmpeak.innerHTML = `<span>PM Peak:</span> ${data.pm_peak || 'N/A'}, ${data.pm_ending || 'N/A'}`
    project.innerHTML = `<span>Project:</span> ${data.project || 'N/A'}`
    countDir.innerHTML = `<span>Count Direction:</span> ${data.counter_direction || 'N/A'}`
    trafficDir.innerHTML = `<span>Traffic Direction:</span> ${data.traffic_direction || 'N/A'}`
...
function create_sidebar_report(data) {
    ...
    tableFooter.classList.add('subheader', 'flex-row', 'flex-align-center', 'flex-wrap')
    ...
    countContext.innerHTML = `<span class="count-type ${data.count_type.toLowerCase()}">${data.count_type} count #${data.record_num}</span>  | ${srOrI} | ${data.municipality.toLowerCase()} | ${data.county.toLowerCase()}`
...
$(window).resize(function() {
    if (tableExists && TCtable.length) {
        table.page.len(Math.max(1, (Math.floor(getDynTableHeight() - 32) / 38.5) - 1)).draw(); // subtract padding and do other things
    }
});
