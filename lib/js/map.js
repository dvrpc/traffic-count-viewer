...
// @WEBPACK: drop request for local file
// $.ajax({
...
// @WEBPACK: ...
// fetch('./data/search_locations.json').then(data => processData(data))
...
function processData(){
        ...
        // searchLocation.forEach((i, location) => {
        //     locationQuery.push({
        //         name: location[1],
        //         value: location[0],
        //         source: location[2]
...
// @TODO: REMOVE and replace with report template fnc
function record_report(dr, typ) {
    //build functionality for various report types
    if (typ === 'Crosswalk') {
        window.open('https://www.dvrpc.org/asp/TrafficCountPDF/' + typ + '/' + dr + '.pdf', '_blank');
    } else if (typ === 'Turning Movement') {
        window.open('https://www.dvrpc.org/asp/TrafficCountPDF/TurningMovement/' + dr + '.pdf', '_blank');
    } else if (typ === 'Manual Class') {
        window.open('https://www.dvrpc.org/asp/TrafficCountPDF/ManualClass/' + dr + '.pdf', '_blank');
    } else if (typ === 'Pedestrian') {
        window.open('https://www.dvrpc.org/asp/pedestriancount/default.aspx?RECNUM='+ dr, '_blank');
    }  else if (typ === 'Bicycle') {
        window.open('https://www.dvrpc.org/asp/bikecount/default.aspx?RECNUM='+ dr, '_blank');
    } 
    else {
        window.open('https://www.dvrpc.org/asp/trafficCount/default.aspx?recnum=' + dr, '_blank');
    }
}

// @UPDATE MAIN: new click events to replace record_report fnc
    // 1: fetch data from API using record num params
    // 2: pass response into create_sidebar_report fnc to create template
    // 3: replace sidebar table w/reponse template
...
async function fetch_report_data(recordNum) {
    ...
    // @NOTE: eventually return response.status for more useful error messages
    return false
}

// https://cloud.dvrpc.org/api/traffic-counts/v1/record/csv/{num}
...
function create_sidebar_report(data) {
    ...
    countType.classList.add('report-type', data.count_type)
    ...
    headerTextWrapper.pappendChild(countType)
...
function remove_sidebar_report(report) {
...
// @UPDATE: add new click handlers here
$(document.body).on('click', '#table tbody td', function() {
    ...
    if (table.cell(this).index().column === 5) {
        ...
        // @UPDATE MAIN: remove record_report
        // record_report(datarow.recnum, datarow.type);

        const response = fetch_report_data(datarow.recnum)

        if(!response) {
            alert(`
                Sorry! Count ID ${datarow.recnum} could not be fetched. Please try again or contact jrocks@dvrpc.org
            `) 
        } else {
            response.then(data => {
                const report = create_sidebar_report(data)
                // hide table (opacity 0? move it out of the way? make it easy to go back...)
