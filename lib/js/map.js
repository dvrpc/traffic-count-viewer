    ...
    onEachFeature: function(feature, layer) {
        if (feature.properties) {
            ...
            layer.on({
                ...
                // @UPDATE: click event for TC features
                click: function(e) {
...
function create_sidebar_report(data) {
    ...
    header.classList.add('flex-row', 'flex-align-end', 'flex-between', 'report-header')
    ...
    back.innerHTML = '&#8592; back to map'
    name.innerHTML = `${data.road} <br> <small>${srOrI} | Count #${data.record_num}</small>`
