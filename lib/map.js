...
function createMap() {
    ...
    L.Control.MapLegend = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            legendDiv.innerHTML += '<div id="legend-icon" title="Toggle Legend"><i class="glyphicon glyphicon-minus"></i></div>';
...
$(document.body).on('click', '#legend-icon', function(){
    ...
    if(toggleStatus === 'closed'){
        ...
        console.log('open');
        $('.map-legend').css('width', '32px').css('height', '32px').attr('data-status', 'closed');
