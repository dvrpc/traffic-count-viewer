...
var searchArea, queryValue, queryLayer, queryName, currTable, rec_query, table, centerLatLng, map, polygonDrawer,
    ...
    table_rows = Math.floor((win_height - 281) / 35);
...
var esri_HYB = L.layerGroup ([L.tileLayer('http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
    attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'
...
function createMap() {
    ...
    var baseLayers = {
        'Satellite (Mapbox)':Mapbox_Imagery,
        'Satellite Hybrid (ESRI w/ Mapquest)':esri_HYB,
...
var records_download = L.geoJson(null, {
    pointToLayer: function(feature, latlng) {
        return L.circleMarker(latlng, {
            stroke: true,
            weight: 1,
            color: ColorLuminance(getValue(feature.properties.TYPE), -0.3),
            radius: 5,
            fillOpacity: 0.8,
...
function get_query_url(layer) {
    switch (layer) {
        ...
        case 'mun_boundary':
            return 'http://arcgis.dvrpc.org/arcgis/rest/services/Boundaries/DVRPC_Boundaries/FeatureServer/2';
        case 'neighborhood':
...
$('#dropdown_types').on('mouseleave', function(e) {
