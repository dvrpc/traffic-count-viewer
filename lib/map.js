    ...
    var TCquery = L.esri.Tasks.query({
            ...
            TCresults.eachLayer(function(m){
                ...
                //    recnum =  m.feature.properties.RECORDNUM,
                    recnum =  m.feature.properties.RECORDNUM + '&nbsp;<i class="glyphicon glyphicon-list-alt" title="download report"></i>',
                    unique = L.stamp(m),
                    dt = new Date(date),
                    MmYear = months[dt.getMonth()] + ' ' + dt.getFullYear(),
                    aadt_form = (aadt != null) ? numeral(aadt).format('0,0') : 'not available'; 
                TCtable.push({"date":MmYear,"road":capitalize(road),"type":capitalize(type),"aadt":aadt_form,"cntdir":cntdir,"unid": unique, "layer":layer,"recnum":recnum});
            });
            ...
            if(tableExists===false){
                table = $('#table').DataTable({
                    ...
                    //    { "data": "cntdir", "title": "Count Direction","width": "5%"},
                        { "data": "type", "title": "Type", "width": "20%"},
                        { "data": "aadt", "title": "AADT*", "width": "14%"},
                    //    { "data": null,"width": "13%", "className": "select-report", "defaultContent": +recnum+"<i class='glyphicon glyphicon-list-alt' title='download report'></i>"},
                        { "data": "recnum","title":"File Number", "width": "13%", "className": "select-report"},
...
$(document.body).on('click', '#table tbody td.select-record', function () {
    ...
    if(check_status.hasClass('glyphicon-check')){
        ...
        download_records.push({"date": row.date,"road": row.road,"type": row.type,"aadt": row.aadt,"cntdir": row.cntdir,"layer": row.layer,"recnum": row.recnum});
...
$(document.body).on('click', '#table tbody td', function () {
    ...
    if(table.cell(this).index().column === 5){
        ...
        var datarow = table.row(trow)
                            .data();
        record_report(datarow.recnum.split('&')[0], datarow.type);
    }
});
