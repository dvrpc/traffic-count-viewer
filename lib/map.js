...
$.getJSON('./data/philly.json', processData);
...
function processData(json){
    ...
    $.each(rows, function(i,location){
      ...
      /*    name: location.attributes.MAPNAME,
            layer: '0',
            value: location.attributes.FID, */
            source: 'locations'
        });
    });
...
/*     url: 'http://services.arcgis.com/rkitYk91zieQFZov/ArcGIS/rest/services/Philadelphia_Neighborhoods/FeatureServer/' + layer  
      }).where("FID='"+query+"'").run(function(error, location){ */
...
function updateSearchArea(zone){
    ...
    var TCquery = L.esri.Tasks.query({
            ...
            TCresults.eachLayer(function(m){
                ...
                TCtable.push({"date":MmYear,"road":capitalize(road),"type":capitalize(type),"aadt":aadt_form,"cntdir":cntdir,"unid": unique, "layer":layer,"recnum":recnum, "report":report});
            });
            ...
            }else{  // if no results all returned clear table and show warning               
