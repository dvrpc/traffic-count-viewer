...
$('div.results-toolbar').html('<button id="view-selected" type="button" class="download_all btn btn-primary btn-xs disabled">view selected</button>&nbsp;&nbsp;<em></em> ');
...
function map_direct() {
    ...
    $('div.results-toolbar').html('<button type="button" class="download-records btn btn-primary btn-xs">download records</button>&nbsp;&nbsp;<p>Showing direct search results. To enable filters identify search area.</p>');
...
function map_results() {
    ...
    $('div.results-toolbar').html('<button id="view-selected" type="button" class="download_all btn btn-primary btn-xs ' + dis + '">view selected</button>&nbsp;&nbsp;<em></em> ');
...
function map_selected() {
    if (download_records.length) {
        ...
        $('div.results-toolbar').html('<button type="button" class="view-all btn btn-primary btn-xs">view all</button><button type="button" class="download-records btn btn-primary btn-xs">download records</button>');
        $('.filter-btn').prop('disabled', true);
    } else {
...
$(document).on('click', '.download-records', function(e) {
    var download_btn_no = 1;
    var download_min = 1;
    var download_max = 50;
    var count_request = '<p>You have requested ' + download_records.length + ' count records for download.</p>';
    count_request += '<a type="button" id="download-1" data-loading-text="Downloading..." class="download-request btn btn-primary btn-md" href="http://www.dvrpc.org/asp/trafficCount/download.aspx?recnum=';
    for (var i = 0, len = download_records.length; i < len; i++){
        if( i + 1 === len && len < 51) {
            count_request += download_records[i].properties.RECORDNUM +'" >Download Records (zip)</a> ';
        } else if ((i + 1 === len || !(i % 49)) && len > 50 && i !== 0){
            count_request += download_records[i].properties.RECORDNUM +'" >Download Records ('+ download_min +' - '+ (i + 1) +')</a>';
        } else if(!(i % 50) && i !== 0) {
            download_btn_no++;
            count_request += '<a type="button" id="download-'+ download_btn_no +'" data-loading-text="Downloading..." class="download-request btn btn-primary btn-md" href="http://www.dvrpc.org/asp/trafficCount/download.aspx?recnum=';
            count_request += download_records[i].properties.RECORDNUM + ',';
            
            download_min = download_max + 1;
            download_max = download_min + 49;
        } else {
            count_request += download_records[i].properties.RECORDNUM + ',';
        }
    }

    $('#download-container').html(count_request);

    $('#download-modal').modal('show');
});
$(document.body).on('click', '.download-request', function() {
    
    $(this).button('loading').delay(4500).queue(function() {
        $(this).button('reset');
        $(this).dequeue();
    });        
